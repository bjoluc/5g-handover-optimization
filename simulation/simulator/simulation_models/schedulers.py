...
class AppAwareResourceFairScheduler(Scheduler):
    ...
    UEs with definite desired datarates
    """
    ...
    def update_data_rates(self, bs, connections):
        ...
        connections = sorted(
            connections,
            key=lambda c: c.ue.desired_datarate / c.maximal_datarate,
        )

        share_per_remaining_ue = 1 / len(connections)

        for scheduled_ues, connection in enumerate(connections):
            desired_datarate = connection.ue.desired_datarate
            maximal_share_datarate = (
                connection.maximal_datarate * share_per_remaining_ue
            )

            if desired_datarate > maximal_share_datarate:
                # The UE takes its full share
                connection.current_datarate = maximal_share_datarate
            else:
                # The UE takes only what it needs and leaves the rest of its share for
                # the remaining UEs
                connection.current_datarate = desired_datarate
                allocated_share_portion = desired_datarate / maximal_share_datarate
